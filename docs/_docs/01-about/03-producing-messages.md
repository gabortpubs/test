...
permalink: /docs/about/producing-messages/
...
You might find it useful to read this information in conjunction with [consuming messages](../consuming-messages) in IBM Event Streams.
...
To connect to IBM Event Streams, you first need to build the set of configuration properties. All connections to IBM Event Streams are secured using TLS and user/password authentication, so you need these properties at a minimum. Replace KAFKA\_BROKERS\_SASL, USER, and PASSWORD with your own credentials:
